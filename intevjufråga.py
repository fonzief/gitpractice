...
def closest_sum_pair(a1, a2, target):
    ...
    while i < len(a1_sorted) and j >= 0:
        ...
        if abs(current_diff) < smallest_diff:
            ...
            #om nuvarande diff blir mindre an minsta diff, ersatts vardet i minsta diff
            closest_pair = (v1, v2)
            #da blir dessa de narmaste paren
#maste kolla upp while loops igen

        if current_diff == 0:
            return closest_pair
            #om diffen blir 0 ar vardet target funnet
        elif current_diff < 0:
            i += 1
            #om inte det blir ett par som blir target kollas narmsta upp och ner
        else:
            j -= 1
    return closest_pair
